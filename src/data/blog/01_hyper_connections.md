...
In a attention block within a transformer architecture, we have a residual connection that connects the input of the block to its output. This can be represented as:
...
$$ X_\ell = [X_{\ell, 1}, \dots, X_{\ell,n_k}]^T \in \mathbb{R}^{n_{h_c} \times d} $$
...
The idea of mHC is to constrain the mapping matrix $B_\ell$ so that its spectral norm is bounded by $1$. It is achieved by constructing $B_\ell$ such that it belons to the set $\mathcal{M}$ given by:
